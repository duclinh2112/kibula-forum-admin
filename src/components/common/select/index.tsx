...
export const Select: SelectComponent = (props) => {
    ...
    return (
      <div className='w-full px-1'>
        {showSearch ? (
          <input
            ref={inputRef}
            className={`text-main bg-white ${!visible ? 'hidden' : 'w-full'}`}
            type='search'
            value={searchKey}
        ...
        <span
          className={`text-main select-none appearance-none truncate ${
      ...
      <>
        {arr && arr.length > 0
          ? arr.map((item: OptionsProps) => {
              return renderMultiple ? (
                ...
                <div
                  key={item.value}
                  className='text-main flex w-fit items-center justify-between gap-2 rounded-lg border border-border-200 px-2 py-1'
                >
        ...
        {showSearch ? (
          <input
            ...
            disabled={disabled || isLoading}
            className={`text-main h-5 flex-1 p-1 focus:outline-none`}
            size={10}
            placeholder={
  ...
  const renderSuffixIcon = () => {
    ...
    return (
      <>
        ...
        <div
          className={`flex h-full items-center justify-center ${isClear && 'group-hover/icon:hidden'} `}
        >
          {suffixIcon || (
          ...
          return (
            ...
            >
              {renderOptions ? (
                ...
                <span className='text-main truncate'>{opt.label}</span>
              )}
              {selected && multiple ? (
                <IconCheck width={18} color='var(--primary-500)' />
              ) : null}
            </li>
          )
        })
  ...
  const normalClass =
    !isLoading && !disabled ? 'cursor-pointer hover:border-primary-500' : ''
  const disabledClass =
