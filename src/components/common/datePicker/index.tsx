...
import type { InterfaceMonthDate, renderSelectedDateProps } from '../calendar'
import { arrMonths, Calendar } from '../calendar'
...
export const DatePicker: React.FC<DatePickerProps> = (props) => {
  ...
  const increaseTime = () => {
    if (type === 'date') {
      const tempDate = moment(values?.toString(), format)
        .add(1, 'days')
        .format(format)
  ...
  const renderMultiple = () => {
    return (
      <div
        style={{ minHeight: 40 }}
        className='flex w-full items-center justify-between gap-1 rounded-lg px-3 py-1'
      >
  ...
  return (
    ...
    >
      ...
      {register && <input className='size-0' {...register} />}
      {container && visible
        ? createPortal(
            <div
              className={'absolute'}
