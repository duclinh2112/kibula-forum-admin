...
import { ColumnProps, DataSourceProps, PAGINATION_POSITION, SORT_ORDER, SortOrderStateProps, TableProps } from './type'
import { useScrollbarWidth } from '@/utils/hooks/useScrollbarWidth'
import IconSortArrowInactive from '@/utils/icons/IconSortArrowInactive'
import IconSortArrowDescending from '@/utils/icons/IconSortArrowDescending'
import IconSortArrowAscending from '@/utils/icons/IconSortArrowAscending'
import { Tooltip } from '../tooltip'
import Expand from '@/utils/icons/IconExpand'
import IconQuestionMark from '@/utils/icons/IconQuestionMark'
...
export const Table: React.FC<TableProps> = ({
  ...
  const _renderHelpIcon = (column: ColumnProps) => {
    ...
    return (
      <Tooltip description={column.helpContent} position='top' showArrow>
        {_renderIcon(
          <IconQuestionMark width={20} className='text-gray-400' />,
        )}
      </Tooltip>
    )
  }
  ...
  const _renderTableCell = (
    ...
    const tdClass = [
      ...
      indexSelected && indexSelected - 1 === rowIndex
        ? 'bg-primary-100'
        : '',
    ...
    <LoadingOverlay loading={loading}>
      <div
        className={`rounded-lg text-sm ${
          layoutBorder
            ? 'overflow-hidden border border-border-200'
            : ''
        } `}
      >
