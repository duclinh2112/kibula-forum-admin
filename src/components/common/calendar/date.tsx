...
import type { CalendarProps, InterfaceMonthDate } from '.'
...
const CalendarDate = (props: ICalendarDate) => {
  ...
  const innerDateNode =
    'flex items-center justify-center rounded-md h-[25px] w-[25px]'
  const disabledInner =
    'before:bg-gray-200 before:w-full before:h-[25px] before:absolute before:opacity-50 before:z-50'
  const selectedInner =
    'before:bg-primary-200 before:w-full before:h-[25px] before:absolute before:z-10'
  ...
  const indexFirstDayCurrentMonth = getIndexFirstDayOfMonth(
    time.year,
    time.month,
  )
  ...
  const checkSelected = (date: string) => {
    if (selectedRange) {
      const validate =
        moment(selectedRange.toDate, format).isSame(
          moment(date, format),
          'day',
        ) ||
        moment(selectedRange.fromDate, format).isSame(
          moment(date, format),
          'day',
        )
      return validate
    ...
    if (value) {
      const validateString = moment(date, format).isSame(
        moment(value, format),
        'day',
      )
    ...
    return sliceDate.map((day, index) => {
      ...
      return (
        ...
        >
          <div
            title={date}
            className={`${innerDateNode} text-text-500 hover:bg-gray-100`}
          >
            {arrayPrevMonth.length - sliceDate.length + index + 1}
          </div>
        </li>
  ...
  const renderDaysInMonth = () => {
    ...
    return arrayCurrentMonth.map((day, index) => {
      ...
      const disabled = isDisabled ? `${disabledStyle} ${disabledInner}` : ''
      let fromAndToCss = ''
      if (isFromDate && haveRange && fromDate !== toDate)
        fromAndToCss = `${fromToDateCss} ${fromDateStyle}`
      if (isToDate && haveRange && fromDate !== toDate)
        fromAndToCss = `${fromToDateCss} ${toDateStyle}`
      const rangeInnerCss = isInRange ? selectedInner : ''
      const fromInnerCss =
        isFromDate && haveRange && fromDate !== toDate
          ? `${selectedInner} ${fromInner}`
          : ''
      const toInnerCss =
        isToDate && haveRange && fromDate !== toDate
          ? `${selectedInner} ${toInner}`
          : ''
      const isRenderCustomSelected =
        renderSelectedDate !== undefined && checkSelected(date)
      const startHoverInnerCss =
        isInHoverRange && (index === 0 || isSunday) ? startHoverInner : ''
      const endHoverInnerCss =
        isInHoverRange && (arrayCurrentMonth.length - 1 === index || isSaturday)
          ? endHoverInner
          : ''
      const hoverInnerCss = isInHoverRange
        ? `${hoverInner} ${startHoverInnerCss} ${endHoverInnerCss}`
        : ''
      if (isDisabled) {
        return (
          <li
            ...
            className={`relative z-50 flex h-[36px] cursor-not-allowed items-center justify-center ${disabled} ${
              isSelected
                ? 'after:border-primary-500 after:absolute after:h-[25px] after:w-[28px] after:rounded after:border'
                : ''
            }`}
            style={{ width: 'calc(100% / 7)' }}
          >
        ...
        >
          <div
            ...
            className={`${selected} ${current} ${innerDateNode} ${
              ...
              isSelected
                ? 'group-hover/date:bg-primary-700'
                : 'group-hover/date:bg-gray-200'
            }`}
          >
            {index + 1}
          </div>
